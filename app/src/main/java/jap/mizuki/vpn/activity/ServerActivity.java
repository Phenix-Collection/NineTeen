...
import jap.mizuki.vpn.util.TotalTraffic;
...
public class ServerActivity extends BaseActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(jap.mizuki.vpn.R.layout.activity_server);
        android.support.v7.app.ActionBar actionBar = getSupportActionBar();
        actionBar.hide();
        mInterstitialAd = new InterstitialAd(this);
        mInterstitialAd.setAdUnitId("ca-app-pub-2803536266877113/8152593708");
        ...
        bookmark = (ImageButton) findViewById(jap.mizuki.vpn.R.id.serverBookmark);
        parentLayout = (LinearLayout) findViewById(jap.mizuki.vpn.R.id.serverParentLayout);
        unblockCheck = (Button) findViewById(jap.mizuki.vpn.R.id.serverUnblockCheck);
        adbBlockCheck = (CheckBox) findViewById(jap.mizuki.vpn.R.id.serverBlockingCheck);
        connectingProgress = (ProgressBar) findViewById(jap.mizuki.vpn.R.id.serverConnectingProgress);
        lastLog = (TextView) findViewById(jap.mizuki.vpn.R.id.serverStatus);
        serverConnect = (Button) findViewById(jap.mizuki.vpn.R.id.serverConnect);

        String totalIn = String.format(getResources().getString(jap.mizuki.vpn.R.string.traffic_in),
                TotalTraffic.getTotalTraffic().get(0));
        trafficInTotally = (TextView) findViewById(jap.mizuki.vpn.R.id.serverTrafficInTotally);
        trafficInTotally.setText(totalIn);

        String totalOut = String.format(getResources().getString(jap.mizuki.vpn.R.string.traffic_out),
                TotalTraffic.getTotalTraffic().get(1));
        trafficOutTotally = (TextView) findViewById(jap.mizuki.vpn.R.id.serverTrafficOutTotally);
        trafficOutTotally.setText(totalOut);

        trafficIn = (TextView) findViewById(jap.mizuki.vpn.R.id.serverTrafficIn);
        trafficIn.setText("");
        trafficOut = (TextView) findViewById(jap.mizuki.vpn.R.id.serverTrafficOut);
        trafficOut.setText("");
        ...
        lastLog.setText(jap.mizuki.vpn.R.string.server_not_connected);
    ...
    private void initView(Intent intent) {
        ...
        int bookmarkBg = dbHelper.checkBookmark(currentServer) ?
                jap.mizuki.vpn.R.drawable.ic_bookmark_red :
                jap.mizuki.vpn.R.drawable.ic_bookmark_grey;
        ...
        ((ImageView) findViewById(jap.mizuki.vpn.R.id.serverFlag))
                .setImageResource(
        ...
        ((TextView) findViewById(jap.mizuki.vpn.R.id.serverCountry)).setText(localeCountryName);
        ((TextView) findViewById(jap.mizuki.vpn.R.id.serverIP)).setText(currentServer.getIp());
        ((TextView) findViewById(jap.mizuki.vpn.R.id.serverCity)).setText(currentServer.getCity());
        ((TextView) findViewById(jap.mizuki.vpn.R.id.serverSessions)).setText(currentServer.getNumVpnSessions());
        ((ImageView) findViewById(jap.mizuki.vpn.R.id.serverImageConnect))
                .setImageResource(
                        getResources().getIdentifier(ConnectionQuality.getConnectIcon(currentServer.getQuality()),
                                "drawable",
                                getPackageName()));

        String ping = currentServer.getPing() + " " +  getString(jap.mizuki.vpn.R.string.ms);
        ((TextView) findViewById(jap.mizuki.vpn.R.id.serverPing)).setText(ping);
        ...
        String speed = String.valueOf(speedValue) + " " + getString(jap.mizuki.vpn.R.string.mbps);
        ((TextView) findViewById(jap.mizuki.vpn.R.id.serverSpeed)).setText(speed);

        if (checkStatus()) {
            adbBlockCheck.setEnabled(false);
            adbBlockCheck.setChecked(filterAds);
            serverConnect.setText(getString(jap.mizuki.vpn.R.string.server_btn_disconnect));
            mInterstitialAd.show();
            ((TextView) findViewById(jap.mizuki.vpn.R.id.serverStatus)).setText(VpnStatus.getLastCleanLogMessage(getApplicationContext()));
        } else {
            serverConnect.setText(getString(jap.mizuki.vpn.R.string.server_btn_connect));
            mInterstitialAd.show();
        }
    }
    ...
    private void receiveTraffic(Context context, Intent intent) {
        if (checkStatus()) {
            ...
            if (firstData) {
                ...
                in = String.format(getResources().getString(jap.mizuki.vpn.R.string.traffic_in),
                        intent.getStringExtra(TotalTraffic.DOWNLOAD_SESSION));
                out = String.format(getResources().getString(jap.mizuki.vpn.R.string.traffic_out),
            ...
            String inTotal = String.format(getResources().getString(jap.mizuki.vpn.R.string.traffic_in),
                    intent.getStringExtra(TotalTraffic.DOWNLOAD_ALL));
            trafficInTotally.setText(inTotal);

            String outTotal = String.format(getResources().getString(jap.mizuki.vpn.R.string.traffic_out),
    ...
    private void prepareVpn() {
        ...
        if (loadVpnProfile()) {
            ...
            connectingProgress.setVisibility(View.GONE);
            Toast.makeText(this, getString(jap.mizuki.vpn.R.string.server_error_loading_profile), Toast.LENGTH_SHORT).show();
        }
    }

    public void serverOnClick(View view) {
        switch (view.getId()) {
            case jap.mizuki.vpn.R.id.serverConnect:
                ...
                break;
            case jap.mizuki.vpn.R.id.serverBtnCheckIp:
                sendTouchButton("serverBtnCheckIp");
                Intent browse = new Intent( Intent.ACTION_VIEW , Uri.parse(getString(jap.mizuki.vpn.R.string.url_check_ip)));
                startActivity(browse);
                break;
            case jap.mizuki.vpn.R.id.serverBookmark:
                sendTouchButton("serverBookmark");

                bookmark.startAnimation(AnimationUtils.loadAnimation(this, jap.mizuki.vpn.R.anim.scale));
                if (dbHelper.checkBookmark(currentServer)) {
                    dbHelper.delBookmark(currentServer);
                    bookmark.setBackground(ContextCompat.getDrawable(this, jap.mizuki.vpn.R.drawable.ic_bookmark_grey));
                } else {
                    dbHelper.setBookmark(currentServer);
                    bookmark.setBackground(ContextCompat.getDrawable(this, jap.mizuki.vpn.R.drawable.ic_bookmark_red));
                }
                break;
        }
    ...
    private void startVpn() {
        ...
        if (intent != null) {
            VpnStatus.updateStateString("USER_VPN_PERMISSION", "", jap.mizuki.vpn.R.string.state_user_vpn_permission,
            ...
            try {
                ...
                VpnStatus.logError(jap.mizuki.vpn.R.string.no_vpn_support_image);
            }
        } else {
    ...
    private void chooseAction() {
        ...
        View view = inflater.inflate(jap.mizuki.vpn.R.layout.pop_up_success_conected,null);
        ...
        Button marketButton = (Button)view.findViewById(jap.mizuki.vpn.R.id.successPopUpBtnPlayMarket);
        ...
        ((Button)view.findViewById(jap.mizuki.vpn.R.id.successPopUpBtnBrowser)).setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
        ...
        ((Button)view.findViewById(jap.mizuki.vpn.R.id.successPopUpBtnSpeedTest)).setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
        ...
        ((Button)view.findViewById(jap.mizuki.vpn.R.id.successPopUpBtnClose)).setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
    ...
    private void showRating() {
        LayoutInflater inflater = (LayoutInflater) getApplicationContext().getSystemService(LAYOUT_INFLATER_SERVICE);
        View view = inflater.inflate(jap.mizuki.vpn.R.layout.pop_up_rating,null);
        ...
        ((Button)view.findViewById(jap.mizuki.vpn.R.id.ratingBtnSure)).setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                popupWindow.dismiss();
                final String appPackageName = getPackageName();
        ...
        ((Button)view.findViewById(jap.mizuki.vpn.R.id.ratingBtnNot)).setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                popupWindow.dismiss();
            }
    ...
    private void showAlert() {
        ...
        builder.setMessage(getString(jap.mizuki.vpn.R.string.try_another_server_text))
                .setPositiveButton(getString(jap.mizuki.vpn.R.string.try_another_server_ok),
                ...
                .setNegativeButton(getString(jap.mizuki.vpn.R.string.try_another_server_no),
                        new DialogInterface.OnClickListener() {
                            public void onClick(DialogInterface dialog, int id) {
                                if (!statusConnection) {
                                    waitConnection = new WaitConnectionAsync();
